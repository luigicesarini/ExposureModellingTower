...
df_tower <- fread('data/tower/df_all_tower_bbox_porto.csv')
...
for (i in 1:length(sf_tower$id)){ #
  ...
  closest_points_roads %>% 
    ...
    sf_tower[i,] %>% 
            ...
            dplyr::select(power,lat,lon,id,contains("GSV"),starts_with("X"),starts_with("Y")) %>% 
   ...
   write.table(line_df_tower,"data/tower/df_4_points_road_distance.csv",
   quote = FALSE, row.names = FALSE, append = TRUE, col.names = !file.exists("data/tower/df_4_points_road_distance.csv"))
}  
)
